...
- **B2 (status logic):**
  ...
  - `unknown` — criterion was found in the tender; the contractor must verify
    their evidence against the quoted requirement.
  - `not_met` — criterion was not found in the pack (potential missing
...
- **B4 (findings):** a `POST` writes or replaces the producer's `qualification_gap`
  findings in the shared `findings` table, severity `high` when `not_met`,
  `medium` when `unknown`.
...
- A1: `POST /api/qualification/opportunities/{id}` returns at least the 8 required
  criteria rows.
- A2: missing criteria are `not_met` with an action-required note.
