...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      if (mode === "signup") await api.signup(email, password, orgName || "My Firm");
    ...
    <div className="mx-auto max-w-md">
      <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {mode === "signup"
            ? "One free full tender review per organisation."
            : "Sign in to your opportunity board."}
        </p>

        <form onSubmit={submit} className="mt-6 space-y-4">
          {mode === "signup" && (
            <Field label="Organisation" value={orgName} onChange={setOrgName} placeholder="Acme Infra Pvt Ltd" />
          )}
        ...
        >
          {mode === "signup" ? "Already have an account? Sign in" : "New here? Create a workspace"}
        </button>
      </div>
    </div>
  );
