...
- **Login workspace flow:** The latest fix calls `switchWorkspace(all[0].workspace_id)` in `login/page.tsx` after MFA, but `SessionProvider.switchWorkspace` closes over the `session` state and returns early because `session` is still `null` immediately after `signIn` returns. The backend `/auth/workspaces/{id}/switch` endpoint itself works and binding the token manually (e.g. via a browser fetch) makes `/billing/settings`, `/support/tickets`, and `/billing/cancel` work. The real fix needs to use a ref or functional update in `SessionProvider` so `switchWorkspace` sees the current `session` token.
