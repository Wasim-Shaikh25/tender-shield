...
|---|---|---|---|---|
| **0** | **Spec + scaffold** | TS-257, TS-258 | `claims.md` agreed; module boots; tables migrated | **in-progress** |
| **1** | **Chronology + checklist** | TS-259, TS-260 | Cited chronology; evidence checklist by claim type | **todo** |
| **2** | **Quantum + delay** | TS-261, TS-262 | Deterministic quantum; delay register with no entitlement conclusion | **todo** |
| **3** | **Drafts + negotiation** | TS-263, TS-264 | Verified-facts drafts; issue→response→negotiation→settlement | **todo** |
| **4** | **Outcome + integrity** | TS-265, TS-266, TS-267 | Recovered value; chain-integrity test; conflict control | **todo** |
...
| TS-258 | `claims` scaffold + migrations | `claims` | P0 | **in-progress** | Tables + boot + list/create claim API | TS-257 |
| TS-259 | Chronology builder | `claims` | P0 | **todo** | Every entry cited; sorted by occurred_at | TS-258 |
| TS-260 | Evidence checklist per claim type | `claims` | P0 | **todo** | Required-type map; present/missing flags | TS-258 |
| TS-261 | Quantum workspace | `claims` | P0 | **todo** | Quantity × rate + daywork; zero LLM | TS-258 |
| TS-262 | Delay-event register | `claims` | P1 | **todo** | Facts only; no entitlement conclusion | TS-258 |
| TS-263 | Draft generators: particulars, variation proposal, EOT, full pack | `claims` + `drafting` | P0 | **todo** | Verified facts + validators + approval | TS-259, TS-261 |
| TS-264 | Issue → response → negotiation → settlement tracking | `claims` | P1 | **todo** | Append-only; status transitions | TS-258 |
| TS-265 | Outcome feedback into private learning set | `claims` + `outcomes` | P0 | **todo** | Publish settlement event | TS-264 |
| TS-266 | Chain-integrity test | `claims` | P0 | **todo** | claim → notice → event → obligation → clause | TS-258 |
