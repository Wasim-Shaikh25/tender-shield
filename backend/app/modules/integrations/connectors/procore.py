"""Procore live connector stub (TS-333)."""
...
class ProcoreConnector(BaseConnector):
    ...
    def authorization_url(
        ...
        client_id = _env("TS_PROCORE_CLIENT_ID")
        auth_url = _env("TS_PROCORE_AUTH_URL") or "https://login.procore.com/oauth/authorize"
        if not client_id:
            return None
        return (
    ...
    def exchange_code(
        ...
        return {
            ...
            "expires_at": None,
            "note": (
                "Procore OAuth code exchange requires TS_PROCORE_CLIENT_SECRET"
    ...
    def fetch(self, source: IntegrationSource) -> dict[str, Any]:
        return {
            ...
            "activities": [],
            "errors": [
                "Procore live fetch is not configured"
                " (TS_PROCORE_BASE_URL and token missing)."
            ],
        }
