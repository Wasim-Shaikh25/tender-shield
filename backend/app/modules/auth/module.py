...
from app.modules.auth.acl import AuthAcl
...
def setup(ctx: AppContext) -> None:
    ...
    ctx.registry.provide(
        "auth.document_class_permitted",
        lambda session, workspace_id, role, document_class: AuthAcl(session).permitted(
            workspace_id, role, document_class
        ),
    )
    ctx.registry.provide(
        "auth.document_class_rules", lambda session: AuthAcl(session)
    )
